...
def handle_read(args: List[str]) -> bool:
    ...
    if not args:
        console.print("❌ Usage: ai_mail read <message_id> or ai_mail read all")
        return False
...
def handle_view(args: List[str]) -> bool:
    ...
    if not args:
        console.print("❌ Usage: ai_mail view <message_id>")
        return False
...
def handle_close(args: List[str]) -> bool:
    ...
    if not args:
        console.print("❌ Usage: ai_mail close <id> [id2 id3 ...] | close all")
        return False
...
def handle_reply(args: List[str]) -> bool:
    ...
    if len(args) < 2:
        console.print("❌ Usage: ai_mail reply <message_id> \"your message\"")
        return False
