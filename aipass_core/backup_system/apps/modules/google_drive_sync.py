...
#   - v2.3.2 (2026-02-10): Fixed deepcopy RuntimeError in save_data during concurrent uploads
#   - v2.3.1 (2026-02-10): Cross-process file locking + retry in load_log/save_log for race condition fix
...
sys.path.insert(0, str(AIPASS_ROOT))  # To ecosystem root
...
def save_data(data: Dict[str, Any]):
    ...
    try:
        JSON_DIR.mkdir(parents=True, exist_ok=True)
        # deepcopy can fail with RuntimeError when concurrent threads mutate
        # the dict during iteration - retry up to 3 times on transient failures
        snapshot = None
        ...
        _atomic_json_write(DATA_FILE, snapshot)
    except Exception as e:
        logger.error(f"Failed to save data: {e}")

def load_log(max_retries: int = 3) -> Dict[str, Any]:
    """Load operation log from JSON file with cross-process locking and retry."""
    default_log = {"entries": [], "summary": {"total_entries": 0, "last_entry": None, "next_id": 1}}
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(f"Failed to load log after {max_retries} attempts: {e}")
                return default_log
        except Exception as e:
...
class GoogleDriveSync:
    ...
    def get_or_create_project_folder(self, project_name: str) -> Optional[str]:
        ...
        # Check if we have a cached folder ID (lock protects concurrent reads/writes)
            ...
            if project_name in self.project_folder_cache:
                ...
                try:
                    if self.drive_service:
                        self._api_call_with_retry(self.drive_service.files().get(fileId=folder_id, fields='id'))
                        return folder_id
                except Exception:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries and self._is_ssl_error(e):
                    wait = 2 ** attempt
                    logger.warning(f"SSL error (attempt {attempt + 1}/{max_retries + 1}), retrying in {wait}s: {e}")
                    time.sleep(wait)
    ...
    def sync_backup_files(self, backup_dir: Path, project_name: str, note: str = "", force_sync: bool = False) -> bool:
        ...
        if upload_count > 0:
            print(f"\033[96m━━━ Phase 2: Upload Changes ({MAX_WORKERS} parallel) ━━━\033[0m")
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                ...
                for future in as_completed(futures):
                    backup_file, result = future.result()
                    with tracker_lock:
                        completed_count += 1
                        if result:
                            success_count += 1
                        print(f"\033[93m↑\033[0m [{completed_count}/{upload_count}] {backup_file.name}")
                        # Batch save: persist tracker and stats every N files
                        if completed_count % BATCH_SAVE_INTERVAL == 0:
                            self._save_file_tracker()
                            save_data(self.data)
                            logger.info(f"Batch save at {completed_count}/{upload_count}")

            # Final save after all uploads complete
            self._save_file_tracker()
            save_data(self.data)
        else:
...
def handle_command(args) -> bool:
    ...
    if command == 'drive-test':
        ...
        print(f"Syncing {backup_path} to Google Drive (project: {project})")
        if force:
            print("Force sync enabled - will upload all files")

        return sync.sync_backup_files(backup_path, project, note, force)

    elif command == 'drive-clear-tracker':
        return clear_file_tracker()

    elif command == 'drive-stats':
        return show_file_tracker_stats()
