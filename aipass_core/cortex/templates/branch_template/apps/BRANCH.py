...
# ===================AIPASS====================
# META DATA HEADER
# Name: {{BRANCH}}.py - {{BRANCHNAME}} Branch Entry Point
# Date: {{DATE}}
# Version: 1.0.0
# Category: {{BRANCH}}
#
# CHANGELOG (Max 5 entries):
#   - v1.0.0 ({{DATE}}): Initial branch creation
#
# CODE STANDARDS:
...
def discover_modules() -> List[Any]:
    ...
    if not MODULES_DIR.exists():
        logger.warning(f"[{{BRANCHNAME}}] Modules directory not found: {MODULES_DIR}")
        return modules
    ...
    for file_path in MODULES_DIR.glob("*.py"):
        ...
        module_name = f"{{BRANCH}}.apps.modules.{file_path.stem}"
        ...
        try:
            ...
            if hasattr(module, 'handle_command'):
                ...
                logger.info(f"[{{BRANCHNAME}}] Loaded module: {file_path.stem}")
            else:
                logger.info(f"[{{BRANCHNAME}}] Skipped {file_path.stem} - no handle_command()")
...
def print_introspection(modules: List[Any]):
    ...
    console.print()
    console.print("[bold cyan]{{BRANCHNAME}} - Branch Management System[/bold cyan]")
    console.print()
    ...
    console.print()
    console.print("[dim]Run 'python3 {{BRANCH}}.py --help' for usage information[/dim]")
    console.print()
...
def print_help(modules: List[Any]):
    ...
    console.print()
    header("{{BRANCHNAME}} - Branch Management System")
    console.print()
    ...
    console.print()
    console.print("  [dim]python3 {{BRANCH}}.py <command> [args...][/dim]")
    console.print("  [dim]python3 {{BRANCH}}.py --help[/dim]")
    console.print()
    console.print("─" * 70)
    console.print()
    ...
    console.print("  [dim]python3 {{BRANCH}}.py <command> --help[/dim]")
    console.print()
...
def main():
    ...
    if route_command(command, remaining_args, modules):
        ...
        console.print()
        console.print("Run [dim]python3 {{BRANCH}}.py --help[/dim] for available commands")
        console.print()
        return 1
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"{{BRANCHNAME}} entry point error: {e}", exc_info=True)
        console.print(f"\n❌ Error: {e}")
        sys.exit(1)
