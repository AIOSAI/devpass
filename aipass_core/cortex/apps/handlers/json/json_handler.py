...
# Constants
CORTEX_ROOT = Path.home() / "aipass_core" / "cortex"
CORTEX_JSON_DIR = CORTEX_ROOT / "cortex_json"
JSON_TEMPLATES_DIR = CORTEX_ROOT / "apps" / "json_templates"
...
def get_json_path(module_name: str, json_type: str) -> Path:
    ...
    return CORTEX_JSON_DIR / filename


def ensure_json_exists(module_name: str, json_type: str) -> bool:
    """Ensure JSON file exists, create from template if missing"""
    CORTEX_JSON_DIR.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    ...
    console.print()
    console.print("[yellow]TESTING:[/yellow] Creating cortex JSONs...")

    # Test auto-creation
    log_operation("test_operation", {"test": "data"}, "cortex")
    increment_counter("cortex", "test_counter", 1)
    update_data_metrics("cortex", test_metric="working")

    console.print()
    console.print("[green]Check /home/aipass/aipass_core/cortex/cortex_json/ for created files:[/green]")
    console.print("  [dim]•[/dim] cortex_config.json")
    console.print("  [dim]•[/dim] cortex_data.json")
    console.print("  [dim]•[/dim] cortex_log.json")
    console.print()
