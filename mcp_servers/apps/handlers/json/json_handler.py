...
# Date: 2025-11-21
# Version: 1.1.0
# Category: mcp_servers/handlers/json
#
...
"""
JSON Handler - Auto-creating JSON file management for MCP_SERVERS branch.

Provides functions for loading, saving, and validating JSON files with
automatic template-based creation when files are missing.
"""
...
# Constants
MCP_SERVERS_ROOT = Path.home() / "mcp_servers"
MCP_SERVERS_JSON_DIR = MCP_SERVERS_ROOT / "mcp_servers_json"
JSON_TEMPLATES_DIR = MCP_SERVERS_ROOT / "apps" / "json_templates"
...
def get_json_path(module_name: str, json_type: str) -> Path:
    ...
    return MCP_SERVERS_JSON_DIR / filename


def ensure_json_exists(module_name: str, json_type: str) -> bool:
    """Ensure JSON file exists, create from template if missing"""
    MCP_SERVERS_JSON_DIR.mkdir(parents=True, exist_ok=True)
    ...
    if json_path.exists():
        try:
            ...
            pass  # Regenerate corrupted file
        except IOError:
            pass  # Regenerate unreadable file
...
if __name__ == "__main__":
    ...
    console.print()
    console.print("[yellow]TESTING:[/yellow] Creating mcp_servers JSONs...")

    # Test auto-creation
    log_operation("test_operation", {"test": "data"}, "mcp_servers")
    increment_counter("mcp_servers", "test_counter", 1)
    update_data_metrics("mcp_servers", test_metric="working")

    console.print()
    console.print("[green]Check /home/aipass/mcp_servers/mcp_servers_json/ for created files:[/green]")
    console.print("  [dim]•[/dim] mcp_servers_config.json")
    console.print("  [dim]•[/dim] mcp_servers_data.json")
    console.print("  [dim]•[/dim] mcp_servers_log.json")
    console.print()
