...
# Date: 2025-11-21
# Version: 1.2.0
# Category: assistant/handlers/json
#
# CHANGELOG (Max 5 entries):
#   - v1.2.0 (2026-01-29): Fixed paths to point to ASSISTANT branch
...
"""
JSON handler for ASSISTANT branch.
...
# Infrastructure
AIPASS_ROOT = Path.home() / "aipass_os" / "dev_central"
sys.path.insert(0, str(AIPASS_ROOT))

# Constants
ASSISTANT_ROOT = Path.home() / "aipass_os" / "dev_central" / "assistant"
ASSISTANT_JSON_DIR = ASSISTANT_ROOT / "assistant_json"
JSON_TEMPLATES_DIR = ASSISTANT_ROOT / "apps" / "json_templates"
...
def get_json_path(module_name: str, json_type: str) -> Path:
    ...
    return ASSISTANT_JSON_DIR / filename


def ensure_json_exists(module_name: str, json_type: str) -> bool:
    """Ensure JSON file exists, create from template if missing."""
    ASSISTANT_JSON_DIR.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    ...
    console.print()
    console.print("[yellow]TESTING:[/yellow] Creating assistant JSONs...")

    log_operation("test_operation", {"test": "data"}, "assistant")
    increment_counter("assistant", "test_counter", 1)
    update_data_metrics("assistant", test_metric="working")

    console.print()
    console.print(f"[green]Check {ASSISTANT_JSON_DIR}/ for created files:[/green]")
    console.print("  [dim]-[/dim] assistant_config.json")
    console.print("  [dim]-[/dim] assistant_data.json")
    console.print("  [dim]-[/dim] assistant_log.json")
    console.print()
