...
# ===================AIPASS====================
# META DATA HEADER
# Name: assistant.py - ASSISTANT Branch Entry Point
# Date: 2026-01-21
# Version: 1.0.0
# Category: assistant
#
...
"""
ASSISTANT Branch - Main Orchestrator
...
# Add dev_central to path for assistant module discovery
DEV_CENTRAL = Path.home() / "aipass_os" / "dev_central"
sys.path.insert(0, str(DEV_CENTRAL))
...
def discover_modules() -> List[Any]:
    ...
    if not MODULES_DIR.exists():
        logger.warning(f"[ASSISTANT] Modules directory not found: {MODULES_DIR}")
        return modules
    ...
    for file_path in MODULES_DIR.glob("*.py"):
        ...
        module_name = f"assistant.apps.modules.{file_path.stem}"
        ...
        try:
            ...
            if hasattr(module, 'handle_command'):
                ...
                logger.info(f"[ASSISTANT] Loaded module: {file_path.stem}")
            else:
                logger.info(f"[ASSISTANT] Skipped {file_path.stem} - no handle_command()")

        except Exception as e:
            logger.error(f"[ASSISTANT] Failed to load module {module_name}: {e}")
...
def route_command(command: str, args: List[str], modules: List[Any]) -> bool:
    ...
    for module in modules:
        try:
            ...
            logger.error(f"[ASSISTANT] Module {module.__name__} error: {e}")
...
def print_introspection(modules: List[Any]):
    ...
    console.print()
    console.print("[bold cyan]ASSISTANT - Branch Management System[/bold cyan]")
    console.print()
    ...
    console.print()
    console.print("[dim]Run 'python3 assistant.py --help' for usage information[/dim]")
    console.print()
...
def print_help(modules: List[Any]):
    ...
    console.print()
    header("ASSISTANT - Branch Management System")
    console.print()
    ...
    console.print()
    console.print("  [dim]python3 assistant.py <command> [args...][/dim]")
    console.print("  [dim]python3 assistant.py --help[/dim]")
    console.print()
    console.print("─" * 70)
    console.print()
    ...
    console.print("  [dim]python3 assistant.py <command> --help[/dim]")
    console.print()
...
def main():
    ...
    if route_command(command, remaining_args, modules):
        ...
        console.print()
        console.print("Run [dim]python3 assistant.py --help[/dim] for available commands")
        console.print()
        return 1
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"ASSISTANT entry point error: {e}", exc_info=True)
        console.print(f"\n❌ Error: {e}")
        sys.exit(1)
