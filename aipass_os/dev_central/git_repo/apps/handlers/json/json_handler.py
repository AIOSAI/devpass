...
# ===================AIPASS====================
# META DATA HEADER
# Name: json_handler.py - JSON Handler for GIT_REPO
# Date: 2025-11-09
# Version: 1.0.1
# Category: git_repo
#
# CHANGELOG (Max 5 entries):
#   - v1.0.1 (2026-01-30): Fixed paths to use branch-local directories
...
Handles default JSON files (config, data, log) for git_repo modules.
...
# Use Python logging (handlers must not import Prax)
import logging
logger = logging.getLogger(__name__)

# Constants
GIT_REPO_ROOT = Path.home() / "aipass_os" / "dev_central" / "git_repo"
GIT_REPO_JSON_DIR = GIT_REPO_ROOT / "git_repo_json"
JSON_TEMPLATES_DIR = GIT_REPO_ROOT / "apps" / "json_templates"
...
def get_json_path(module_name: str, json_type: str) -> Path:
    ...
    return GIT_REPO_JSON_DIR / filename


def ensure_json_exists(module_name: str, json_type: str) -> bool:
    """Ensure JSON file exists, create from template if missing"""
    GIT_REPO_JSON_DIR.mkdir(parents=True, exist_ok=True)
    ...
    if json_path.exists():
        try:
            ...
            # Invalid JSON - will recreate from template
            pass
        except OSError:
            # File access error - will recreate from template
            pass
...
if __name__ == "__main__":
    ...
    console.print("="*70)
    console.print("\n[TESTING] Creating git_repo JSONs...")

    # Test auto-creation
    log_operation("test_operation", {"test": "data"}, "git_repo")
    increment_counter("git_repo", "test_counter", 1)
    update_data_metrics("git_repo", test_metric="working")

    console.print("\nCheck /home/aipass/aipass_os/dev_central/git_repo/git_repo_json/ for created files:")
    console.print("  - git_repo_config.json")
    console.print("  - git_repo_data.json")
    console.print("  - git_repo_log.json")
    console.print("\n" + "="*70 + "\n")
