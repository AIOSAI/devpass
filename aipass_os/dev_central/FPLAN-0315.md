# FPLAN-0315 - Medic Phase 2 - Per-Branch Mute/Unmute
...
### Goal
Add per-branch mute/unmute to the Medic toggle system. When a branch is muted, Medic still detects errors in that branch's logs but does NOT dispatch auto-healing agents. Muted branches are tracked in config and persist across restarts.

### Approach
Dispatch to @trigger - they own the Medic system (built Phase 1). Trigger creates their own Flow plan, builds the feature, tests thoroughly, and reports back.

### Requirements
- `drone @trigger medic mute @speakeasy` - mute a specific branch
- `drone @trigger medic unmute @speakeasy` - unmute a specific branch
- `drone @trigger medic status` - shows muted branches list in status output
- Muted branches list persists in trigger_config.json (or medic_state.json)
- error_detected.py checks muted list before dispatching
- Muted errors still logged to medic_suppressed.log with reason "branch muted"
- DEV_CENTRAL can never be added as a dispatch target (existing protection, verify it holds)

### Reference Documents
- `dev_planning/prax/auto_healing_toggle.md` - Full Medic planning doc
- Phase 1 files: `trigger/apps/modules/medic.py`, `trigger/apps/handlers/medic_state.py`, `trigger/apps/handlers/events/error_detected.py`, `trigger/trigger_json/trigger_config.json`
...
```bash
drone @ai_mail send @dev_central "PRODUCTION STOPPED: FPLAN-0315" "Issue: [description]. Attempted: [what was tried]. Awaiting guidance."
```
...
- [ ] Status email sent to DEV_CENTRAL:
  ```bash
  drone @ai_mail send @dev_central "FPLAN-0315 Complete" "Summary of what was done, any issues, outcomes"
  ```
