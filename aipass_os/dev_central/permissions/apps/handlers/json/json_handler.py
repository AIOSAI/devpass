...
# Date: 2025-11-21
# Version: 1.1.0
# Category: permissions/handlers/json
#
...
# =============================================
"""
JSON Auto-Creating Handler for Permissions branch.

Provides automatic JSON file creation and management with template support,
validation, and log rotation capabilities.
"""
...
# Constants
PERMISSIONS_ROOT = Path.home() / "aipass_os" / "dev_central" / "permissions"
PERMISSIONS_JSON_DIR = PERMISSIONS_ROOT / "permissions_json"
JSON_TEMPLATES_DIR = PERMISSIONS_ROOT / "apps" / "json_templates"
...
def get_json_path(module_name: str, json_type: str) -> Path:
    ...
    return PERMISSIONS_JSON_DIR / filename


def ensure_json_exists(module_name: str, json_type: str) -> bool:
    """Ensure JSON file exists, create from template if missing"""
    PERMISSIONS_JSON_DIR.mkdir(parents=True, exist_ok=True)
    ...
    if json_path.exists():
        try:
            ...
            # If unreadable or corrupted, fall through to regenerate
            pass
...
if __name__ == "__main__":
    ...
    console.print()
    console.print("[yellow]TESTING:[/yellow] Creating permissions JSONs...")

    # Test auto-creation
    log_operation("test_operation", {"test": "data"}, "permissions")
    increment_counter("permissions", "test_counter", 1)
    update_data_metrics("permissions", test_metric="working")

    console.print()
    console.print("[green]Check /home/aipass/aipass_os/dev_central/permissions/permissions_json/ for created files:[/green]")
    console.print("  [dim]•[/dim] permissions_config.json")
    console.print("  [dim]•[/dim] permissions_data.json")
    console.print("  [dim]•[/dim] permissions_log.json")
    console.print()
