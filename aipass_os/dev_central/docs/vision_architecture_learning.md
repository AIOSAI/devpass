...
```
AIPass-Ecosystem/
├── Python skills (OpenAI, memory, file ops)
├── Node.js help system  
...
- Overview → Sections → Line comments → Character highlighting
...
├── Directory structure (flow/, prax/, skills/, etc.)
├── Standard patterns (3-file JSON, logging architecture)  
├── Core orchestration (drone commands, registry system)
└── Base infrastructure (system_logs/, memory_bank/)

Installable Modules (The Components)
├── aipass-backup      # pip install aipass-backup
├── aipass-flow        # pip install aipass-flow
├── aipass-skills-api  # pip install aipass-skills-api
├── aipass-memory      # pip install aipass-memory
├── aipass-prax        # pip install aipass-prax
└── aipass-[component] # pip install aipass-[component]
```
...
aipass list
  ✅ aipass-flow (subscribed)
  ✅ aipass-backup (subscribed)
  🔒 aipass-enterprise-memory (upgrade required)
...
pip install --upgrade aipass-backup  # v1.2 → v1.3
...
- Backup system → aipass-backup
- Flow/PLAN system → aipass-flow
- Skills framework → aipass-skills
- Prax infrastructure → aipass-prax
- API modules → aipass-api
...
```
AIPass Module Store:
├── Official Modules (by AIPass team)
├── Community Modules (verified)
├── Enterprise Modules (premium)
└── Custom Modules (private repos)
```
...
**For Development:**
- ✅ 5-minute deployments instead of 40+
- ✅ Test components in isolation
- ✅ No more "80% success then failure"
- ✅ Clean ignore pattern management

**For Distribution:**
- ✅ Users install only what they need
- ✅ Automatic updates when available
- ✅ Public/private module options
- ✅ Multiple installation methods

**For Business:**
- ✅ Subscription model ready
- ✅ Enterprise customization possible
...
Human <-> Orchestration Layer
    ...
    ├── File watcher AI (continuous)
    ├── Documentation AI (automatic)
    ├── Memory curator AI (background)
    ├── Code optimizer AI (post-process)
    ├── Pattern recognition AI (learning)
    └── Presentation layer AI (filtering)
```
...
**Previous Risk:**
- Rename `scratch.md` → `notepad.md` in template
...
**Result:**
- Template changes `scratch.md` → `notepad.md`
- Update detects: "f008 changed name" → **rename** operation
...
```
{{BRANCH}}/
├── apps/                    ← Execution code (universal convention)
├── tests/                   ← Validation (pytest with conftest.py)
├── tools/                   ← Utilities
├── DOCUMENTS/               ← Long-term AI memory (JSON soon)
├── logs/                    ← History (dual: local + system)
├── .backup/                 ← Safety net (automatic)
├── .archive/                ← Pruned files
├── {{BRANCH}}_json/         ← Branch-specific data
├── ai_mail.local/           ← Inter-branch messaging
├── STANDARDS.local/         ← Branch-specific standards (NEW)
│
├── BRANCH.ID.json           ← Identity (placeholder → TEST_RENAME.id.json)
├── LOCAL.json               ← Session tracking
├── OBSERVATIONS.json        ← Collaboration patterns
├── AI_MAIL.json             ← Messages
├── CLAUDE.local.md          ← Plan summaries (auto-updated)
├── README.json              ← Auto-documentation (JSON now)
├── dev.local.md             ← Patrick's notes (NOT loaded)
└── .branch_meta.json        ← File tracking registry (NEW)
```

**Placeholder System:**
- `{{BRANCH}}` in filenames → `TEST_RENAME` (uppercase)
- `{{BRANCH}}` in paths → `test_rename_json` (lowercase for dirs)
...
```
Each Branch:
  STANDARDS.local/code_standards.json  ← Branch manages autonomously

Admin Branch:
  /home/aipass/standards/GLOBAL.json   ← Aggregator with placeholders
...
**Critical Discovery:**
- `branch_update.py` reported: "❌ Failed: 10/17"
...
```
✅ Added/Updated: 6
ℹ️  Skipped: 10 (already exist)
📦 Backup: /path/to/backup

✅ Update complete!  # Only if error_count == 0
```
...
3. API branch next session: 📨 notification
4. Small bug? → Fix autonomously
5. Send confirmation back

**Current infrastructure:**
- ✅ AI Mail transport layer
- ✅ Notifications (📨 indicator)
- ✅ Branch registry (knows all branches)
- ❌ Decision logic (autonomous vs. escalate)
...
```
Rollover System:
  Local.json (600 lines) → Compress → MEMORY_BANK
  DOCUMENTS (10 max) → Oldest archived → MEMORY_BANK
  PLAN files → Already process there (TRL tags)
...
Future State:
  ...
  - "Hey, didn't we solve this 6 months ago?" → searchable
...
- File tracking registry → enables safe template evolution
- Standards federation → enables autonomous improvement
- Memory bank → enables collective learning
- AI Mail → enables inter-branch coordination
...
- Skip ≠ Error (critical distinction)
...
- Memory federation (local → documents → memory bank)
- Inter-branch communication (AI Mail)
- Automated documentation (JSON → aggregation)
...
- Template→branch transformation safe
...
- Phase 1-5 refactoring complete (update_branch.py: 1,164→843 lines)
...
- CI/CD pipeline (test → validate → deploy to all branches)
- Quality gate (nothing ships without validation)
- Template distributor (one change → 70 branches updated)
...
- System dirs (.cache, .npm, node_modules) → ignore
- AIPass branches (aipass_core, aipass_dev) → scan
...
**Pattern:** When you see logic in wrong place → move to handler. Separation enables evolution.
...
│
├── You walk in, see the directory:
│
├── aipass_core/          ← Core operations (70 branches here)
├── aipass_business/      ← Business profile (separate ecosystem)
├── input_x/              ← Input-X profile (separate ecosystem)
├── speakeasy/            ← External tool (voice interface)
├── marketplace/          ← External discovery system
├── mcp_servers/          ← External integrations
├── standards_hub/        ← Aggregation hub (preferred over "central")
├── plans_hub/            ← Aggregation hub
└── devpulse_hub/         ← Aggregation hub
```
...
✅ How to track files (IDs + hashes)
✅ How to detect changes (renames vs adds vs prunes)
✅ How to sync reality with registry (scan .id.json)
✅ How to handle placeholders (source vs destination)
✅ How to ignore the right things (backup vs registry patterns)
✅ How to protect user data (never overwrite Python)
...
Push through cortex → 70 branches updated consistently.
...
**The Numbers:**
- 17 branches registered → cleaned to reality
- 70 branches actually exist → "only bare bones"
...
- Need better change detection? → work on change_detection.py
- Need better backup? → work on file_ops.py
- Need registry improvements? → work on registry.py
...
- Registry auto-sync ✅
- Template validation ✅
- Backup system with ignores ✅
- ID-based tracking ✅
- **A framework that can scale** ✅
...
├── central/
│   ├── aggregation.py, sync.py, branch_list.py
├── bulletin/
│   ├── storage.py, crud.py, propagation.py
└── dashboard/
    ├── operations.py, status.py
```
...
- Hierarchy is built for orchestration (conductor → orchestra → specialists)
...
- "Hey, we have 20 websites to build today" → "Yeah, no problem"
...
```
        ...
        ▼         ▼         ▼           ▼         ▼         ▼
    ┌───────┐ ┌───────┐ ┌───────┐ ┌───────┐ ┌───────┐ ┌───────┐
    │  API  │ │Website│ │Office │ │ Games │ │LangChn│ │Playgnd│
...
**API Building Door:**
"Hey, I want to build some APIs today" → Walk through the door, AI support ready, tools configured, framework guidance available.
...
**Office Door:**
"I want to build an Excel spreadsheet with X, Y, and Z" → Office section with all options, AI assistance for automation, templates, integrations.
