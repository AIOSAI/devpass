...
# Date: 2026-02-11
# Version: 1.2.0
# Category: speakeasy/handlers/json
#
# CHANGELOG (Max 5 entries):
#   - v1.2.0 (2026-02-11): Updated paths for Speakeasy branch
...
"""
JSON Auto-Creating Handler for Speakeasy

Provides automatic JSON file management with template-based creation,
validation, and persistence. Handles config, data, and log JSON files
for all Speakeasy modules.
"""
...
# Constants
SPEAKEASY_ROOT = Path.home() / "speakeasy"
SPEAKEASY_JSON_DIR = SPEAKEASY_ROOT / "speakeasy_json"
JSON_TEMPLATES_DIR = SPEAKEASY_ROOT / "apps" / "json_templates"
...
def load_template(json_type: str, module_name: str) -> Any:
    ...
    template_str = template_str.replace("2026-02-11", datetime.now().date().isoformat())
...
def get_json_path(module_name: str, json_type: str) -> Path:
    ...
    return SPEAKEASY_JSON_DIR / filename


def ensure_json_exists(module_name: str, json_type: str) -> bool:
    """Ensure JSON file exists, create from template if missing"""
    SPEAKEASY_JSON_DIR.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    ...
    console.print()
    console.print("[yellow]TESTING:[/yellow] Creating speakeasy JSONs...")

    # Test auto-creation
    log_operation("test_operation", {"test": "data"}, "speakeasy")
    increment_counter("speakeasy", "test_counter", 1)
    update_data_metrics("speakeasy", test_metric="working")

    console.print()
    console.print("[green]Check /home/aipass/speakeasy/speakeasy_json/ for created files:[/green]")
    console.print("  [dim]•[/dim] speakeasy_config.json")
    console.print("  [dim]•[/dim] speakeasy_data.json")
    console.print("  [dim]•[/dim] speakeasy_log.json")
    console.print()
