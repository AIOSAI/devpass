"""PARTICK_PRIVATE handlers package - Security protected."""

import inspect
from pathlib import Path

MY_BRANCH = "partick_private"
        ...
        for frame in stack:
            if frame.filename in ("<string>", "<stdin>"):
                ...
                raise ImportError(
                    ...
                    f"    from partick_private.apps.modules.<module> import <function>\n"
                    f"{'='*60}"
                )
        return

    if f"/partick_private/" in caller_file:
        return
