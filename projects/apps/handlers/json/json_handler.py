...
# Date: 2025-11-21
# Version: 1.1.0
# Category: projects/handlers/json
#
...
Handles default JSON files (config, data, log) for projects modules.
...
PROJECTS_JSON_DIR = PROJECTS_ROOT / "projects_json"
JSON_TEMPLATES_DIR = PROJECTS_ROOT / "apps" / "json_templates"
...
def get_json_path(module_name: str, json_type: str) -> Path:
    ...
    return PROJECTS_JSON_DIR / filename


def ensure_json_exists(module_name: str, json_type: str) -> bool:
    """Ensure JSON file exists, create from template if missing"""
    PROJECTS_JSON_DIR.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    ...
    console.print()
    console.print("[yellow]TESTING:[/yellow] Creating projects JSONs...")

    # Test auto-creation
    log_operation("test_operation", {"test": "data"}, "projects")
    increment_counter("projects", "test_counter", 1)
    update_data_metrics("projects", test_metric="working")

    console.print()
    console.print("[green]Check /home/aipass/projects/projects_json/ for created files:[/green]")
    console.print("  [dim]•[/dim] projects_config.json")
    console.print("  [dim]•[/dim] projects_data.json")
    console.print("  [dim]•[/dim] projects_log.json")
    console.print()
